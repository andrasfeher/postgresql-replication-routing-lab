...
#
if [[ ! -d "$CONFIG" ]]; then
    echo "[node2] Creating Debian PostgreSQL cluster configuration"
...
if [[ "$NEEDS_BASEBACKUP" == true ]]; then
    echo "[node2] Removing temporary data directory created by pg_createcluster"
...
if [[ ! -f "${PGDATA}/PG_VERSION" ]]; then
    echo "[node2] Initializing standby from primary"

    echo "[node2] Waiting for primary"
    ...
    echo "[node2] Running pg_basebackup"
    ...
    echo "[node2] Base backup completed"
fi
...
echo "[node2] Starting PostgreSQL standby"
